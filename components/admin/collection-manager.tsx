...
import { createCollectionAction, deleteCollectionAction, updateCollectionAction } from "@/lib/actions/admin/taxonomy";
...
export function CollectionManager({ collections }: { collections: Collection[] }) {
  ...
  function startEdit(collection: Collection) {
    ...
    setForm({ name: collection.name, slug: collection.slug, description: collection.description ?? "", image: collection.image ?? "" });
    setShowForm(true);
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = editingId ? await updateCollectionAction(editingId, form) : await createCollectionAction(form);
      if (result.success) {
        push(editingId ? "Collection updated" : "Collection created", "success");
        setForm(emptyForm);
        setEditingId(null);
        setShowForm(false);
        router.refresh();
    ...
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {collections.map((collection) => (
          <div key={collection.id} className="rounded-[1.5rem] border border-[#eadac2] bg-white p-5">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-medium text-[#1b120d]">{collection.name}</p>
                <p className="text-xs text-[#8a7469]">/{collection.slug}</p>
              </div>
              <div className="flex gap-2">
                <button onClick={() => startEdit(collection)} className="rounded-full border border-[#dcc7ad] p-1.5 text-[#4f3e36]">
                  <Pencil size={13} />
                </button>
                <button
                  disabled={pending}
                  onClick={() =>
                    startTransition(async () => {
                      ...
                      if (result.success) {
                        push("Collection removed", "success");
                        router.refresh();
                      } else {
                        push(result.error ?? "Could not remove collection.", "error");
                      }
                    })
                  }
            ...
            {collection.description ? <p className="mt-2 text-sm text-[#5d4d45]">{collection.description}</p> : null}
          </div>
        ))}
      </div>
